...
export const InventoryTable = ({ inventory = [] }) => { const rows = Array.isArray(inventory) ? inventory : []; return <div className="table-responsive"><table className="table inventory-table"><thead><tr><th>Blood Group</th><th>Units Available</th></tr></thead><tbody>{rows.map(item => <tr key={item._id || item.bloodGroup}><td>{item.bloodGroup}</td><td>{item.unitsAvailable}</td></tr>)}</tbody></table></div>; };
